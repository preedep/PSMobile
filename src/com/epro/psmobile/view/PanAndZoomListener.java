...
public class PanAndZoomListener implements OnTouchListener {
  ...
  public PanAndZoomListener(RelativeLayout containter, View view, int anchor) {
      ...
      if (child instanceof ImageView && ((ImageView) child).getScaleType()== ImageView.ScaleType.MATRIX) {
		...
		SvgRaster.svgAndroidRenderToArea(
			...
			0, 0, canvas.getWidth(), canvas.getHeight());	
        	   */
        	  Picture pic = ((PictureDrawable)drawable).getPicture();
        	  if (pic != null)
        	  {
        		  float bmWidth = pic.getWidth();
        		  float bmHeight = pic.getHeight();
